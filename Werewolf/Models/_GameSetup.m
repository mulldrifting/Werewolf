...
const struct GameSetupAttributes GameSetupAttributes = {
	.isDefault = @"isDefault",
	.kDayKillAnnounced = @"kDayKillAnnounced",
	.kNightKillAnnounced = @"kNightKillAnnounced",
	.kWolvesSeeRoleOfKill = @"kWolvesSeeRoleOfKill",
	.name = @"name",
...
+ (NSSet*)keyPathsForValuesAffectingValueForKey:(NSString*)key {
	...
	if ([key isEqualToString:@"kDayKillAnnouncedValue"]) {
		NSSet *affectingKey = [NSSet setWithObject:@"kDayKillAnnounced"];
		keyPaths = [keyPaths setByAddingObjectsFromSet:affectingKey];
		return keyPaths;
	}
	if ([key isEqualToString:@"kNightKillAnnouncedValue"]) {
		NSSet *affectingKey = [NSSet setWithObject:@"kNightKillAnnounced"];
		keyPaths = [keyPaths setByAddingObjectsFromSet:affectingKey];
		return keyPaths;
	}
	if ([key isEqualToString:@"kWolvesSeeRoleOfKillValue"]) {
		NSSet *affectingKey = [NSSet setWithObject:@"kWolvesSeeRoleOfKill"];
		keyPaths = [keyPaths setByAddingObjectsFromSet:affectingKey];
		return keyPaths;
	}
...
@dynamic kDayKillAnnounced;



- (BOOL)kDayKillAnnouncedValue {
	NSNumber *result = [self kDayKillAnnounced];
	return [result boolValue];
}

- (void)setKDayKillAnnouncedValue:(BOOL)value_ {
	[self setKDayKillAnnounced:[NSNumber numberWithBool:value_]];
}

- (BOOL)primitiveKDayKillAnnouncedValue {
	NSNumber *result = [self primitiveKDayKillAnnounced];
	return [result boolValue];
}

- (void)setPrimitiveKDayKillAnnouncedValue:(BOOL)value_ {
	[self setPrimitiveKDayKillAnnounced:[NSNumber numberWithBool:value_]];
}
...
@dynamic kNightKillAnnounced;



- (BOOL)kNightKillAnnouncedValue {
	NSNumber *result = [self kNightKillAnnounced];
	return [result boolValue];
}

- (void)setKNightKillAnnouncedValue:(BOOL)value_ {
	[self setKNightKillAnnounced:[NSNumber numberWithBool:value_]];
}

- (BOOL)primitiveKNightKillAnnouncedValue {
	NSNumber *result = [self primitiveKNightKillAnnounced];
	return [result boolValue];
}

- (void)setPrimitiveKNightKillAnnouncedValue:(BOOL)value_ {
	[self setPrimitiveKNightKillAnnounced:[NSNumber numberWithBool:value_]];
}
...
@dynamic kWolvesSeeRoleOfKill;



- (BOOL)kWolvesSeeRoleOfKillValue {
	NSNumber *result = [self kWolvesSeeRoleOfKill];
	return [result boolValue];
}

- (void)setKWolvesSeeRoleOfKillValue:(BOOL)value_ {
	[self setKWolvesSeeRoleOfKill:[NSNumber numberWithBool:value_]];
}

- (BOOL)primitiveKWolvesSeeRoleOfKillValue {
	NSNumber *result = [self primitiveKWolvesSeeRoleOfKill];
	return [result boolValue];
}

- (void)setPrimitiveKWolvesSeeRoleOfKillValue:(BOOL)value_ {
	[self setPrimitiveKWolvesSeeRoleOfKill:[NSNumber numberWithBool:value_]];
}
